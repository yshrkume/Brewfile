...
- `Brewfile.lock.json` - Lock file tracking exact versions and commit hashes of taps (generated by `scripts/update-brewfile-lock.py`)
...
- Homebrew Bundle does not generate a lock file; this repo updates it via `scripts/update-brewfile-lock.py`
...
- **Lock file conflicts**: Remove `Brewfile.lock.json` and regenerate with `python3 scripts/update-brewfile-lock.py`
...
5. **Lock file update**: Run `python3 scripts/update-brewfile-lock.py` after Brewfile changes
6. **Environment management**: Consider using environment variables for different scenarios
...
4. Update lock file with `python3 scripts/update-brewfile-lock.py` [macOS-only]
